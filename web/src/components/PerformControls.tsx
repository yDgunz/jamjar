              ...
              <>
                ...
                <div className="mt-2 flex items-center justify-between gap-4">
                  ...
                  <button
                    ...
                    className={`relative h-6 w-10 rounded-full transition-colors ${wrapText ? "bg-accent-600" : "bg-gray-700"}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform ${wrapText ? "translate-x-4" : ""}`} />
                  </button>
                </div>
                <div className="mt-2 flex items-center justify-between gap-4">
                  <span className="text-sm text-gray-400">Lyrics only</span>
                  <button
                    role="switch"
                    aria-checked={lyricsOnly}
                    onClick={() => setLyricsOnly((v) => !v)}
                    className={`relative h-6 w-10 rounded-full transition-colors ${lyricsOnly ? "bg-accent-600" : "bg-gray-700"}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white transition-transform ${lyricsOnly ? "translate-x-4" : ""}`} />
                  </button>
                </div>
              </div>
              </>
